...
<template>
  <div
    ref="sidebar"
    :class="{
      'fixed top-0 hidden pt-20 lg:flex lg:w-60 xl:w-80 h-screen':
        mode === 'normal',
    ...
    <div class="flex-1 overflow-y-auto pl-4 lg:pl-0 pr-4 py-4">
      <ul>
        <li v-for="i in 29" :key="i">
          ...
          >
            <div
              class="flex items-center mr-4 px-2 py-2 rounded-sm dark:group-hover:highlight-white/10 dark:highlight-white/10"
              :class="{
                'text-white dark:text-white group-hover:bg-primary-500 bg-primary-500':
                  i === 1,
            ...
            <span
              ...
              :class="{
                'font-extrabold text-primary-500 dark:text-primary-400': i === 1,
              }"
            >
